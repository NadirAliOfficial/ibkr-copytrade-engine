...
class NeroAIClient:
    ...
    def _build_ui(self):
        ...
        # logo — exact PNG, trimmed, no black bg
        ...
        cp.pack(fill="x", padx=24, pady=(4, 12))
        ci = tk.Frame(cp, bg=PANEL, padx=20, pady=16)
        ci.pack(fill="x")
        ...
        lf.pack(fill="both", expand=True, padx=24, pady=(0,20))
        lf_hdr = tk.Frame(lf, bg=BORDER, padx=16, pady=7)
        lf_hdr.pack(fill="x")
        ...
        self.log = scrolledtext.ScrolledText(lf, font=("Courier New", 9), bg=BG, fg=TEXT,
        ...
        self._log("NeroAI CopyTrade Engine ready. Configure connection and press START.", "info")
    ...
    def _start(self):
        ...
        self._log(f"Connecting to TWS  {host}:{port}  clientId={cid} …", "info")
    ...
    def _run(self, host, port, cid):
        ...
        try:
            ...
            self._log("TWS connected ✓", "ok")
            self._fetch_balance()
        except Exception as e:
            self.root.after(0, lambda: self._log(f"Connection failed: {e}", "err"))
        ...
        while not self._stop.is_set():
            try:
                ...
                self.root.after(0, lambda e=e: self._log(f"VPS poll error: {e}", "err"))
            time.sleep(2)
    ...
    def _maybe_place(self, o):
        key = f"{o['symbol']}-{o['action']}-{o['quantity']}-{o['price']}"
        if key in self.placed: return
        self.placed.add(key)
        self.root.after(0, lambda: self._inc("received"))
        try:
            contract = Stock(o["symbol"], o["exchange"], o["currency"])
            order = MarketOrder(o["action"], o["quantity"]) if o["orderType"] == "MKT"                     else LimitOrder(o["action"], o["quantity"], o["price"])
            self.ib.placeOrder(contract, order)
            self.root.after(0, lambda: self._inc("placed"))
            msg = f"{o['action']:4s}  {o['symbol']:<6s}  qty={o['quantity']}  {o['orderType']} @ {o['price']}"
            self.root.after(0, lambda m=msg: self._log(m, "ok"))
        except Exception as e:
            self.root.after(0, lambda: self._inc("failed"))
            self.root.after(0, lambda e=e: self._log(f"Order error: {e}", "err"))
